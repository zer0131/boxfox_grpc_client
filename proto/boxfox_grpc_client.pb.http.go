...
var (
	httpBoxfoxGrpcClientServiceServer BoxfoxGrpcClientServiceServer
)

func RegisterHTTPBoxfoxGrpcClientServiceServer(srv BoxfoxGrpcClientServiceServer) {
	httpBoxfoxGrpcClientServiceServer = srv
}
...
var _HTTP_Path_Handler = map[string]httpMethodHandler{
	...
	"/demo.BoxfoxGrpcClientService/Monitor": _HTTP_BoxfoxGrpcClientService_Monitor_Handler,
}
...
func _HTTP_BoxfoxGrpcClientService_Monitor_Handler(ctx context.Context, req interface{}) (interface{}, error) {
	var protoReq MonitorReqNew
	if err := json.Unmarshal(req.([]byte), &protoReq); err != nil {
		return nil, err
	}
	return httpBoxfoxGrpcClientServiceServer.Monitor(ctx, &protoReq)
}
