...
import (
	...
	"boxfox_grpc_client/handler"
	"boxfox_grpc_client/hook"
...
func main() {
	...
	if reflect.ValueOf(app.ConfigVal.BaseVal).FieldByName("LogExpireDay").IsValid() {
		err = log.InitV4(log.WithProject("boxfox_grpc_client"), log.WithMaxLength(app.ConfigVal.BaseVal.LogSize), log.WithExpireDay(reflect.ValueOf(app.ConfigVal.BaseVal).FieldByName("LogExpireDay").Int()))
	} else {
		err = log.InitV4(log.WithProject("boxfox_grpc_client"), log.WithMaxLength(app.ConfigVal.BaseVal.LogSize))
	}
	// 等待log写入文件完毕再退出
	defer log.Close()
	...
	hook := &hook.DpHook{}
	rpcBoxfoxGrpcClientServiceHandler := &handler.BoxfoxGrpcClientServiceHandler{}
	// handler -> service -> model 依赖注入
	injectHandler(hook, rpcBoxfoxGrpcClientServiceHandler)
	...
	if !app.ConfigVal.BaseVal.DisableWebServer {
		...
		startGrpc(ctx, grpcL, rpcBoxfoxGrpcClientServiceHandler)
		startHttp(ctx, addr, httpL, rpcBoxfoxGrpcClientServiceHandler)
...
func injectHandler(hook *hook.DpHook, rpcBoxfoxGrpcClientServiceHandler *handler.BoxfoxGrpcClientServiceHandler) {
	injector := inject.New()
	...
	if err := injector.Apply(&rpcBoxfoxGrpcClientServiceHandler); err != nil {
		// 参考spring，直接panic
		panic(err)
	}
...
func startGrpc(ctx context.Context, grpcL net.Listener, rpcBoxfoxGrpcClientServiceHandler *handler.BoxfoxGrpcClientServiceHandler) {
	...
	pb.RegisterBoxfoxGrpcClientServiceServer(grpcSrv, rpcBoxfoxGrpcClientServiceHandler)
...
func startHttp(ctx context.Context, addr string, httpL net.Listener, rpcBoxfoxGrpcClientServiceHandler *handler.BoxfoxGrpcClientServiceHandler) {
	http.Handle("/static/", http.StripPrefix("/static/", http.FileServer(http.Dir("./static"))))
	http.Handle("/", &pb.HTTPHandler{})

	pb.RegisterHTTPBoxfoxGrpcClientServiceServer(rpcBoxfoxGrpcClientServiceHandler)
